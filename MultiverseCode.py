...
import os
os.chdir('/home/jafri/mount_dir')
from pathlib import Path
PROJECT_ROOT = Path.cwd()
data_path = PROJECT_ROOT / 'Dataset'
output_path = PROJECT_ROOT / 'Output_new2'
...
df = pd.read_csv('/home/jafri/mount_dir/score_factor.csv')
...
#This is how the subject will be input and printed when running on the cluster
...
def low_pass_filter(datal):
    ...
    #lpfimpulse is optional: only for visualization
    lpfimpulse = np.empty((np.size(datal)-2, datal[0][0].shape[0], datal[0][0].shape[1]))

#There was data from 4 sessions. run in [0,2] will take data from session 1 and 2, whereas run in [2,4] will take data from session 3 and 4.

    for run in [0,2]:#range(np.size(datal)): Change to []
...
#Necessary to change [0,2] to [2,4] here if you want data from session 3 and 4. For the rest of the code.

    for run in [0,2]:#range(np.size(datab)): #
        ...
        for region in range(x2_values.shape[0]):
            ...
            if run == 0:
                ...
                bpf[run-1, region, :] = filtered_data
...
def calculate_functional_connectivity(datafc):
    ...
    for run in range(datafc.shape[0]):
        ...
        # Compute wavelet correlation matrix for discrete wavelet transform
...
# For extracting plots: can be commented out if not needed
import statsmodels.api as sm
sm.graphics.plot_corr(Waveletfc_wgsr_lp[0])
plt.title('Subject ' + str(3) + ' Run1-Wavelet correlation-WGSR')
plt.show()
...
# plt.imshow(Waveletfc_gsr_lp[0], cmap='RdBu_r', aspect='auto')
# plt.colorbar()
# plt.title('Wavelet Coherence Run' + str(1))
# plt.show()
